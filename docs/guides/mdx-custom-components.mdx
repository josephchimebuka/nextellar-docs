...
- Prefer Markdown for normal prose, headings, lists, tables, and links.
- Use custom components when the page needs interaction, a preview, a callout, or a specialized visual.
- Keep examples small enough to read in the docs page where they appear.
...
```mdx
<Note type="tip">
  Use `pnpm build:content` after adding or moving MDX files.
</Note>
...
<CodeTabs
  tabs={{
    tsx: { syntax: "const wallet = useStellarWallet();", language: "tsx" },
    js: { syntax: "const wallet = useStellarWallet();", language: "js" },
  }}
/>
...
<CustomSyntaxHighlighter
  ...
  tabs={{
    bash: { syntax: "pnpm build:content", language: "bash" },
  }}
/>
```
...
<FolderTree>
  <Folder element="docs" defaultOpen={true}>
    <Folder element="guides">
      <File>mdx-custom-components.mdx</File>
    </Folder>
  </Folder>
</FolderTree>
```
...
| Component | Props | Constraints |
| --- | --- | --- |
| `Button` | `variant?: "primary" \| "secondary" \| "outline" \| "none"`, `size?: "xs" \| "sm" \| "md" \| "lg" \| "xl"`, standard `button` props | Use for interactive examples or mock controls. |
| `Input` | `variant?: "outline" \| "filled" \| "none"`, `size?: "xs" \| "sm" \| "md" \| "lg" \| "xl"`, `fullWidth?: boolean`, `isError?: boolean`, `isLoading?: boolean`, standard `input` props | Avoid collecting real secrets in documentation examples. |
| `Checkbox` | Standard checkbox `input` props | Use `defaultChecked` for static examples. |
| `Label` | `intent?: "default" \| "muted" \| "error"`, standard `label` props | Pair with an input `id` when labeling a control. |
| `Select` | `multiple?: boolean`, `search?: boolean`, `value?: string \| string[]`, `onSelect?`, `className?`, `disabled?` | Must wrap `SelectValue`, `SelectContent`, and `SelectItem`. |
| `SelectValue` | `placeholder?: string`, standard `div` props | Must be inside `Select`. |
| `SelectContent` | Standard `div` props | Must be inside `Select`. |
| `SelectItem` | `value: string`, standard `div` props | `value` is used for selection and search filtering. |
...
<Dialog>
  ...
  <DialogContent>
    <DialogHeader>
      ...
      <DialogDescription>Review the selected Stellar network.</DialogDescription>
    </DialogHeader>
...
<Popover position="bottom">
  <PopoverTrigger>
    <Button variant="outline">Endpoint</Button>
  </PopoverTrigger>
  <PopoverContent className="p-3">
    Horizon testnet endpoint
    <PopoverClose className="mt-2 text-sm">Close</PopoverClose>
  </PopoverContent>
</Popover>
...
<Menu>
  ...
  <PopMenu>
    <MenuItem>Copy endpoint</MenuItem>
    <MenuItem>Open reference</MenuItem>
  </PopMenu>
</Menu>
```
...
| Component | Props | Constraints |
| --- | --- | --- |
| `NavMenu` | `children`, `className?` | Use for compact navigation previews. |
| `NavMenuList` | Standard `ul` props | Must contain `NavMenuItem` children. |
| `NavMenuItem` | Standard `li` props | Provides hover group behavior for content. |
| `NavMenuTrigger` | Standard `button` props | Place before matching content. |
| `NavMenuContent` | `position?: "up" \| "down"`, standard `div` props | Must be inside `NavMenuItem`. |
| `NavListItem` | `href: string`, `title: string`, `children` | Renders a linked list item for `NavMenuContent`. |

```mdx
<NavMenu>
  <NavMenuList>
    <NavMenuItem>
      <NavMenuTrigger>Guides</NavMenuTrigger>
      <NavMenuContent>
        <ul className="grid gap-2 p-3">
          <NavListItem href="/docs/guides/add-docs-page" title="Add a page">
            Create and link a new documentation route.
          </NavListItem>
        </ul>
      </NavMenuContent>
    </NavMenuItem>
